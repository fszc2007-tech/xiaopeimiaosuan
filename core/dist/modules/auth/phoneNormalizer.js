 ...
 * @returns 规范化结果
 */
function normalizePhone(rawPhone, countryCode, expectedRegion) {
    try {
        // 1. 拼接完整手机号
        ...
        // 3. 解析手机号
        const phoneNumber = (0, libphonenumber_js_1.parsePhoneNumber)(fullPhone, defaultCountry);
        // 4. 验证手机号有效性
        ...
        if (expectedRegion && detectedRegion !== expectedRegion) {
            ...
            console.warn(`[phoneNormalizer] Region mismatch: expected=${expectedRegion}, detected=${detectedRegion}, phone=${e164Phone}`);
 ...
 * 从检测到的国家代码获取地区类型
 */
function getRegionFromDetectedCode(detectedCode) {
    const mapping = {
        '+86': 'cn',
        '+852': 'hk',
        '+853': 'mo',
