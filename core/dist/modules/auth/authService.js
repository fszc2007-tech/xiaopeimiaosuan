...
async function requestOTP(params) {
    ...
    const [recentSends] = await pool.execute(`SELECT code_id, code, created_at 
     FROM verification_codes 
     WHERE phone = ? 
       AND created_at > DATE_SUB(NOW(), INTERVAL 2 MINUTE)
     ORDER BY created_at DESC`, [normalizedPhone]);
    // 🔍 诊断：记录数据库指纹（检查读写分离/复制延迟）
    const [dbFingerprint] = await pool.execute(`SELECT DATABASE() AS db, @@hostname AS host, @@server_id AS sid, @@read_only AS ro, @@time_zone AS tz, NOW() AS db_now`);
    // 🔍 修复：使用数据库 DATE_ADD 函数生成 expires_at，确保时区一致
    await pool.execute(`INSERT INTO verification_codes (code_id, phone, code, code_type, expires_at, is_used) 
     VALUES (?, ?, ?, 'login', DATE_ADD(NOW(), INTERVAL ? MINUTE), FALSE)`, [codeId, normalizedPhone, code, ttlMinutes]);
    // 查询实际插入的 expires_at（用于日志）
    const [insertedRow] = await pool.execute(`SELECT expires_at FROM verification_codes WHERE code_id = ?`, [codeId]);
...
async function loginOrRegister(params) {
    ...
    // 2. 验证验证码（从数据库查询）
    // 🔍 诊断：记录数据库指纹（检查读写分离/复制延迟）
    const [dbFingerprint] = await pool.execute(`SELECT DATABASE() AS db, @@hostname AS host, @@server_id AS sid, @@read_only AS ro, @@time_zone AS tz`);
    const fingerprint = {
        db: dbFingerprint[0]?.db,
        host: dbFingerprint[0]?.host,
        serverId: dbFingerprint[0]?.sid,
    ...
    if (codeRows.length === 0) {
        ...
        const [candidateRows] = await pool.execute(`SELECT id, phone, code, code_type, is_used, created_at, expires_at 
       FROM verification_codes 
       WHERE phone = ? 
       ORDER BY created_at DESC 
    ...
    if (userRows.length > 0) {
        ...
        await pool.execute(`INSERT INTO users (user_id, nickname, phone, app_region, is_pro) 
       VALUES (?, ?, ?, ?, FALSE)`, [userId, nickname, normalizedPhone, channel.toUpperCase()]);
        // 重新查询用户信息
