 ...
 * 计算命盘
 */
async function computeChart(params) {
    ...
    if (!profileId) {
        ...
        await pool.execute(`INSERT INTO chart_profiles 
    ...
    // 2. 检查是否已有计算结果（且版本匹配）
    if (!forceRecompute) {
        const [chartRows] = await pool.execute(`SELECT * FROM bazi_charts 
       WHERE chart_profile_id = ? AND engine_version = ? AND needs_update = FALSE
       ORDER BY created_at DESC LIMIT 1`, [profileId, CURRENT_ENGINE_VERSION]);
    ...
    // 4. 调用八字引擎计算
    const result = await (0, engine_1.computeBaziChart)(engineBirthInfo);
    // 5. 保存计算结果
    const chartId = (0, uuid_1.v4)();
    await pool.execute(`INSERT INTO bazi_charts (chart_id, chart_profile_id, result_json, engine_version)
     VALUES (?, ?, ?, ?)`, [chartId, profileId, JSON.stringify(result), CURRENT_ENGINE_VERSION]);
    return {
        chartId,
        chartProfileId: profileId,
        result,
...
async function getCharts(params) {
    ...
    // 查询总数
    const [countRows] = await pool.execute(`SELECT COUNT(*) as total FROM chart_profiles 
     WHERE user_id = ? AND name LIKE ?`, [userId, `%${search}%`]);
    // 查询列表
    // ✅ 修复 MySQL 8 + mysql2 兼容性问题：直接拼接 LIMIT/OFFSET（已校验数字安全）
    // 注意：LIMIT/OFFSET 直接拼接到 SQL 中，不使用占位符（避免 MySQL 8 兼容性问题）
    const [rows] = await pool.execute(`SELECT * FROM chart_profiles 
     WHERE user_id = ? AND name LIKE ?
     ORDER BY created_at DESC
     LIMIT ${safePageSize} OFFSET ${offset}`, [userId, `%${search}%`]);
    return {
        items: rows,
 ...
 * 获取命盘详情
 */
async function getChartDetail(params) {
    const { userId, chartId } = params;
    const pool = (0, connection_1.getPool)();
    // 查询命盘结果
    const [chartRows] = await pool.execute(`SELECT bc.*, cp.* 
     FROM bazi_charts bc
    ...
    let birthInfo = resultJson.birthInfo;
    ...
    if (resultJson.derived?.luck_cycle) {
        const { buildLuckCycleForApp } = await Promise.resolve().then(() => __importStar(require('../../utils/luckCycleAdapter')));
        luckCycle = buildLuckCycleForApp(resultJson.derived.luck_cycle, currentAge);
    }
    ...
    if (analysis.luckRhythm && !analysis.luckRhythm.annualBrief) {
        try {
            // 动态导入 ES 模块
            const annualLuckModule = await Promise.resolve().then(() => __importStar(require('../../../engine/analysis/annualLuck.js')));
 ...
 */
function calculateCurrentAge(birthInfo) {
...
async function deleteChart(params) {
    ...
    let profileId = null;
    const [chartRows] = await pool.execute(`SELECT bc.chart_profile_id 
     FROM bazi_charts bc
     JOIN chart_profiles cp ON bc.chart_profile_id = cp.chart_profile_id
     WHERE bc.chart_id = ? AND cp.user_id = ?`, [chartId, userId]);
    if (chartRows.length > 0) {
        ...
        const [profileRows] = await pool.execute(`SELECT chart_profile_id 
       FROM chart_profiles 
       WHERE chart_profile_id = ? AND user_id = ?`, [chartId, userId]);
    ...
    await pool.execute(`DELETE FROM chart_profiles WHERE chart_profile_id = ?`, [profileId]);
}
