 ...
 * 根据中文名称获取神煞代码
 */
function getShenshaCode(chineseName) {
    // 去掉括号
 ...
 * 生成推荐提问（简单模板，不需要太详细）
 */
function generateRecommendedQuestions(shenshaName, pillarType) {
    const pillarLabels = {
        'year': '年柱',
        'month': '月柱',
        'day': '日柱',
 ...
 * 转换单条数据
 */
function convertShenshaData(data) {
    const sqlStatements = [];
    // 获取神煞代码
    ...
    // 处理四个柱位
    const pillars = [
        { type: 'year', label: '年柱', text: data.年柱 },
        { type: 'month', label: '月柱', text: data.月柱 },
        { type: 'day', label: '日柱', text: data.日柱 },
        { type: 'hour', label: '时柱', text: data.时柱 },
    ];
 ...
 * 主函数：转换所有数据
 */
function convertAllData(userData) {
    const allSql = [];
...
//# sourceMappingURL=convert-shensha-data.js.map