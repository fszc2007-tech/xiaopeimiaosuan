...
async function checkLLMMode() {
    ...
    try {
        // 1. 检查默认模型
        const [defaultRows] = await pool.query(`SELECT model FROM llm_api_configs WHERE is_default = TRUE AND is_enabled = TRUE LIMIT 1`);
        ...
        console.log(`\n📌 默认模型: ${defaultModel}\n`);
        // 2. 检查 DeepSeek 配置
        const [deepseekRows] = await pool.query(`SELECT model, is_enabled, is_default, thinking_mode, model_name 
       ...
       WHERE model = 'deepseek'`);
        ...
        console.log('\n📊 所有 LLM 配置:');
        console.log('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━');
        const [allRows] = await pool.query(`SELECT model, is_enabled, is_default, thinking_mode 
       FROM llm_api_configs 
...
//# sourceMappingURL=check-llm-mode.js.map