...
function extractSpecPaths() {
    const specFile = path_1.default.join(__dirname, '../../app.doc/API接口统一规范.md');
    if (!fs_1.default.existsSync(specFile)) {
        console.warn('[Warning] API接口统一规范.md not found');
        return [];
    }
    const content = fs_1.default.readFileSync(specFile, 'utf-8');
    const paths = [];
    // 正则匹配：#### 5.1.1 登录 / 注册
    // - **方法**: `POST`
...
function checkAlignment() {
    ...
    for (const spec of specPaths) {
        const found = registeredPaths.find(reg => reg.method === spec.method && reg.path === spec.path);
        if (!found) {
    ...
    for (const reg of registeredPaths) {
        const found = specPaths.find(spec => spec.method === reg.method && spec.path === reg.path);
        if (!found) {
...
//# sourceMappingURL=checkApiPaths.js.map