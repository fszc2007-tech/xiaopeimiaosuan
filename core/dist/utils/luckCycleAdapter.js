...
exports.buildLuckCycleForApp = buildLuckCycleForApp;
// ===== 核心转换函数 =====
/**
 * 将引擎原始大运数据转换为前端格式
 */
function buildLuckCycleForApp(rawLuckCycle, currentAge) {
 ...
 */
function mapFavourLevel(rawFavour, favourScore) {
    if (!rawFavour) {
        // 兼容旧数据：一律按平稳处理
        return 'flat';
    }
 ...
 * 获取 toneTag（含向后兼容）
 */
function getToneTag(level, shishen) {
    if (!shishen) {
 ...
 * 获取 keywords（含向后兼容）
 */
function getKeywords(favourLevel, shishen) {
    if (!shishen) {
...
//# sourceMappingURL=luckCycleAdapter.js.map