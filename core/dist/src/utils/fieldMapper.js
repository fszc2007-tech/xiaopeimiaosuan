...
class FieldMapper {
    ...
    // ===== 对话相关 =====
    static mapConversationItem(row) {
        return {
    ...
    static mapSubscriptions(rows) {
        return rows.map(row => this.mapSubscription(row));
    }
    // ===== Admin 用户相关 =====
    static mapAdminUser(row) {
        return {
    ...
    // ===== LLM 配置相关 =====
    static mapLLMConfig(row, decryptedKey) {
        return {
