...
async function googleLogin(params) {
    ...
    const subHash = crypto_1.default.createHash('sha256').update(providerUserInfo.sub).digest('hex').substring(0, 16);
    ...
    try {
        ...
        // #endregion
        const [identityRows] = await connection.query('SELECT * FROM auth_identities WHERE provider = ? AND provider_user_id = ?', ['google', providerUserInfo.sub]);
        // #region agent log
        ...
        if (identityRows && identityRows.length > 0) {
            ...
            if (updateFields.length > 0) {
                ...
                await connection.query(`UPDATE auth_identities SET ${updateFields.join(', ')} WHERE identity_id = ?`, updateValues);
            }
            // 2.2 返回关联的用户
            const [userRows] = await connection.query('SELECT * FROM users WHERE user_id = ?', [identityRows[0].user_id]);
            await connection.commit();
            if (!userRows || userRows.length === 0) {
                throw new Error('User not found for identity');
            }
            const user = userRows[0];
            const userDto = fieldMapper_1.FieldMapper.mapUser(user);
        ...
        // 3.1 创建用户（不绑定手机号）
        await connection.query('INSERT INTO users (user_id, email, nickname, avatar_url, app_region, created_at, updated_at) VALUES (?, ?, ?, ?, ?, NOW(), NOW())', [
            userId,
        ...
        try {
            await connection.query('INSERT INTO auth_identities (identity_id, user_id, provider, provider_user_id, email, name, avatar_url, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, NOW(), NOW())', [
                identityId,
                userId,
                'google',
            ...
            if (insertError.code === 'ER_DUP_ENTRY') {
                ...
                const [existingIdentityRows] = await pool.query('SELECT * FROM auth_identities WHERE provider = ? AND provider_user_id = ?', ['google', providerUserInfo.sub]);
                if (existingIdentityRows && existingIdentityRows.length > 0) {
                    const [existingUserRows] = await pool.query('SELECT * FROM users WHERE user_id = ?', [existingIdentityRows[0].user_id]);
        ...
        connection.release();
        // 查询新创建的用户
        const [newUserRows] = await pool.query('SELECT * FROM users WHERE user_id = ?', [userId]);
        if (!newUserRows || newUserRows.length === 0) {
            throw new Error('Failed to create user');
        }
        const user = newUserRows[0];
        const userDto = fieldMapper_1.FieldMapper.toUserDto(user);
        const token = generateToken(user.user_id);
        return {
            token,
            user: userDto, // 返回完整的 UserDto，包含 status 和 deleteScheduledAt
...
//# sourceMappingURL=thirdPartyAuthService.js.map