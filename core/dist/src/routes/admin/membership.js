...
const adminMembershipService_1 = require("../../modules/admin/adminMembershipService");
...
router.get('/users', async (req, res) => {
    try {
        ...
        const result = await (0, adminMembershipService_1.getMembershipUserList)(page, pageSize, q, isPro);
        res.json({
            success: true,
            data: result,
        });
...
router.get('/users/:userId', async (req, res) => {
    try {
        ...
        const result = await (0, adminMembershipService_1.getMembershipUserDetail)(userId);
        res.json({
            success: true,
            data: result,
        });
 ...
 * Admin 開通 / 延長會員
 */
router.post('/users/:userId/grant', async (req, res) => {
    try {
        const { userId } = req.params;
        ...
        const result = await (0, adminMembershipService_1.grantMembership)(userId, plan, mode);
        res.json({
            success: true,
            data: result,
        });
...
router.post('/users/:userId/revoke', async (req, res) => {
    try {
        const { userId } = req.params;
        const result = await (0, adminMembershipService_1.revokeMembership)(userId);
        res.json({
            success: true,
            data: result,
        });
 ...
 * Admin 重置今日 AI 次數
 */
router.post('/users/:userId/reset-ai-today', async (req, res) => {
    try {
        const { userId } = req.params;
        const result = await (0, adminMembershipService_1.resetTodayAiCalls)(userId);
        res.json({
            success: true,
            data: result,
        });
