 ...
 * 提取基础信息
 */
function extractBasicInfo(chartResult: any): ExamChatContext['basic'] {
 ...
 * 推导学习天赋等级
 */
function deriveStudyTalentLevel(chartResult: any): '偏弱' | '普通' | '较强' | '很强' {
  const analysis = chartResult.analysis || {};
 ...
 * 推导学习风格
 */
function deriveFocusStyle(chartResult: any): string {
  const analysis = chartResult.analysis || {};
  const structure = analysis.structure || {};
  const W = structure.W || structure.tenGodWeights || {};
 ...
 * 推导压力承受力
 */
function deriveStressResistance(chartResult: any): '偏弱' | '中等' | '较强' {
  const analysis = chartResult.analysis || {};
  const strength = analysis.strengthAnalysis || {};
  const structure = analysis.structure || {};
  const strengthLabel = strength.label || strength.band || '';
 ...
 * 推导坚持度
 */
function derivePersistenceLevel(chartResult: any): '容易放弃' | '波动型' | '能长期坚持' {
  const analysis = chartResult.analysis || {};
  const structure = analysis.structure || {};
  const W = structure.W || structure.tenGodWeights || {};
  const yinWeight = (W.yin || 0) + (W.zYin || 0) + (W.pYin || 0);
 ...
 * 推导考试基础友好度
 */
function deriveExamLuckBaseline(chartResult: any): string {
  const analysis = chartResult.analysis || {};
  const structure = analysis.structure || {};
  const W = structure.W || structure.tenGodWeights || {};
  const yongshenPattern = analysis.yongshenPattern || {};
 ...
 * 推导学习与心理特质
 */
function deriveTalentInfo(chartResult: any): ExamChatContext['talent'] {
  return {
 ...
 * 推导适合学术/研究程度
 */
function deriveFitAcademia(chartResult: any): '低' | '中' | '高' {
  const analysis = chartResult.analysis || {};
  const structure = analysis.structure || {};
  const W = structure.W || structure.tenGodWeights || {};
  const structureName = structure.name || '';
 ...
 * 推导适合体制/公职程度
 */
function deriveFitCivilService(chartResult: any): '低' | '中' | '高' {
  const analysis = chartResult.analysis || {};
  const structure = analysis.structure || {};
  const W = structure.W || structure.tenGodWeights || {};
  const structureName = structure.name || '';
 ...
 * 推导核心标签
 */
function deriveCoreTags(chartResult: any, direction: ExamChatContext['direction']): string[] {
  const analysis = chartResult.analysis || {};
  const structure = analysis.structure || {};
  const W = structure.W || structure.tenGodWeights || {};
 ...
 * 推导职业路径倾向
 */
function deriveDirectionInfo(chartResult: any): ExamChatContext['direction'] {
 ...
 * 构建时间轴信息
 */
function buildTimelineInfo(chartResult: any, now: Date): ExamChatContext['timeline'] {
  const analysis = chartResult.analysis || {};
  ...
  function mapFavourLevelToExam(favLevel: string, shishen?: string): 'golden' | 'good' | 'normal' | 'hard' {
 ...
 * 提取辅助分析信息
 */
function extractExtraInfo(chartResult: any): ExamChatContext['extra'] {
  const analysis = chartResult.analysis || {};
  const dayMaster = analysis.dayMaster || {};
  const structure = analysis.structure || {};
  const strength = analysis.strengthAnalysis || {};
 ...
 * 判断主题类型
 */
function detectTopic(question: string, plan: ExamChatContext['plan']): 'postgrad' | 'civil' | 'both' | 'other' {
 ...
 * 判断年龄段提示
 */
function deriveAgeStageHint(chartResult: any, now: Date): string {
  const analysis = chartResult.analysis || {};
  const luckRhythm = analysis.luckRhythm || {};
  const currentAge = luckRhythm.currentAge || 0;
 ...
 * 判断模式
 */
function detectMode(plan: ExamChatContext['plan'], topic: 'postgrad' | 'civil' | 'both' | 'other'): 'exam' | 'postgrad' | 'civil' | 'other' {
  if (topic === 'postgrad') {
    return 'postgrad';
  } else if (topic === 'civil') {
    return 'civil';
 ...
 */
export async function buildExamChatContext(params: {
  chartResult: any;
