...
import { LoveChatContext, PillarLoveInfo } from '../../types/love';

/**
 * 检查用户问题中是否提到伴侣
 */
function detectPartnerMention(question: string): boolean {
  const keywords = [
 ...
 * 提取恋爱相关神煞
 */
function extractLoveShenSha(shenshaList: string[]): string[] {
  const loveShenSha = [
    '桃花', '咸池', '红鸾', '天喜', '流霞', '孤辰', '寡宿', 
    '天乙贵人', '月德', '天德'
  ];
  
  return (shenshaList || []).filter((s: string) => 
 ...
 * 提取四柱恋爱信息
 */
function extractPillarsLoveInfo(chartResult: any): LoveChatContext['pillars'] {
 ...
 */
async function buildPalaceContext(chartResult: any): Promise<LoveChatContext['palace']> {
  const pillars = chartResult.pillars || {};
  ...
  // 提取与日支相关的关系
  const dayRelations = {
    he: (relationships.liuhe || []).filter((r: any) => 
      (r.branch1 === dayBranch || r.branch2 === dayBranch)
    ).map((r: any) => `${r.branch1}${r.branch2}六合`),
    
    sanhe: (relationships.sanhe || []).filter((r: any) => 
      (r.branches || []).includes(dayBranch)
    ).map((r: any) => r.description || '三合'),
    
    chong: (relationships.liuchong || []).filter((r: any) => 
      (r.branch1 === dayBranch || r.branch2 === dayBranch)
    ).map((r: any) => `${r.branch1}${r.branch2}六冲`),
    
    xing: (relationships.sanxing || []).filter((r: any) => 
      (r.branches || []).includes(dayBranch)
    ).map((r: any) => r.description || '三刑'),
    
    hai: (relationships.liuhai || []).filter((r: any) => 
      (r.branch1 === dayBranch || r.branch2 === dayBranch)
    ).map((r: any) => `${r.branch1}${r.branch2}六害`),
  };
 ...
 * 生成恋爱环境提示
 */
function buildLoveEnvironmentNotes(pillars: any, relationships: any): string {
  const notes: string[] = [];
  ...
  if (monthShenSha.some((s: string) => s.includes('桃花')) || 
      hourShenSha.some((s: string) => s.includes('桃花'))) {
 ...
 */
async function buildSpouseStarDistribution(
  pillars: any,
  dayMasterGan: string,
  spouseStarType: '财星' | '官杀'
): Promise<string> {
  const utilsModule = await import('../../../engine/analysis/utils.js');
 ...
 * 构建配偶星与表达方式信息
 */
async function buildSpouseAndExpression(
  chartResult: any,
  gender: 'male' | 'female' | 'unknown'
): Promise<LoveChatContext['spouseAndExpression']> {
  ...
  // 构建配偶星分布
  const spouseStarDistribution = await buildSpouseStarDistribution(
    pillars,
    dayMasterGan,
    spouseStarType
  );
 ...
 * 映射喜忌等级到数字
 */
function mapFavourLevel(favourLevel: any): number {
 ...
 * 计算流年与配偶宫的关系
 */
async function calcYearBranchRelations(
  yearBranch: string,
  spouseBranch: string
): Promise<string[]> {
  if (!yearBranch || !spouseBranch) return [];
  ...
  if (he) relations.push(`${yearBranch}${spouseBranch}六合`);
  
  // 六冲
  const chong = checkLiuChong(yearBranch, spouseBranch);
  if (chong) relations.push(`${yearBranch}${spouseBranch}六冲`);
  
  // 三刑
  const xing = checkSanXing(yearBranch, spouseBranch);
  if (xing) relations.push(`${yearBranch}${spouseBranch}三刑`);
  
  // 六害
  const hai = checkLiuHai(yearBranch, spouseBranch);
  if (hai) relations.push(`${yearBranch}${spouseBranch}六害`);
  
  // 相破
  const po = checkXiangPo(yearBranch, spouseBranch);
  if (po) relations.push(`${yearBranch}${spouseBranch}相破`);
 ...
 * 提取流年恋爱神煞
 */
function extractYearLoveShenSha(yearData: any): string[] {
 ...
 * 构建大运流年信息
 */
async function buildFortuneContext(
  chartResult: any,
  spouseBranch: string,
  now: Date
): Promise<LoveChatContext['fortune']> {
  const analysis = chartResult.analysis || {};
  ...
  for (const item of filteredYears) {
    ...
    const flowYear = derived.flow_years?.find((y: any) => y.year === item.year);
 ...
 */
export async function buildLoveChatContext(params: {
  chartResult: any;
  gender: 'male' | 'female' | 'unknown';
  userQuestion?: string;
  now?: Date;
