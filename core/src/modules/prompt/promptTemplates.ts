 ...
 * 小佩系统人设 Prompt
 */
export const XIAOPEI_SYSTEM_PROMPT = `你是「小佩」，一位专业的命理 AI 助手，你的职责是帮助用户理解他们的八字命盘，并提供基于命理学的指导和建议。
 ...
 * - 强调自然聊天、直接回答
 */
export const XIAOPEI_SYSTEM_PROMPT_CHAT = `你是「小佩」，一位专业的命理 AI 助手。
 ...
 */
export const XIAOPEI_OUTPUT_STYLE = `
## 輸出排版要求（非常重要）
 ...
 * 神煞解读 Prompt
 */
export function buildShenshaPrompt(params: {
  shenshaCode: string;
  shenshaName: string;
  userQuestion: string;
  baziData: any;
 ...
 * 让 LLM 在聊天时自己发挥会更自然
 */
export function buildChartSummary(baziData: any): string {
...
export function buildGeneralPrompt(params: {
  ...
  if (conversationHistory && conversationHistory.length > 0) {
    historyText = '\n## 对话历史\n' + conversationHistory.map(msg => 
      `${msg.role === 'user' ? '用户' : '小佩'}: ${msg.content}`
...
function buildPalaceSixKinPromptCore(params: {
  ...
  dayPalace: string;
  hourPalace: string;
  pillarStrengthSummary: string;
  sixKinKeywords: string[];
  notes?: string;
  userQuestion?: string;
}): string {
  const {
...
function buildPalaceSixKinPrompt(params: {
  ...
  if (!pillarStrengthSummary) {
    ...
    const pillarStrengths: string[] = [];
    if (strengthMap.year) pillarStrengths.push(`年柱${strengthMap.year}`);
    if (strengthMap.month) pillarStrengths.push(`月柱${strengthMap.month}`);
    if (strengthMap.day) pillarStrengths.push(`日柱${strengthMap.day}`);
    if (strengthMap.hour) pillarStrengths.push(`时柱${strengthMap.hour}`);
  ...
  if (structure) {
    ...
    if (shensha?.hits_by_pillar) {
      const hasTaohua = Object.values(shensha.hits_by_pillar).some((hits: any) => 
        Array.isArray(hits) && hits.some((h: string) => h.includes('桃花') || h.includes('咸池'))
      );
      if (hasTaohua) {
