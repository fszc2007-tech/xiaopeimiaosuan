...
export type FollowupScene =
  // 场景卡片（6 个）
  | 'love'           // 恋爱·桃花（topic: 'love' | 'LOVE'）
  | 'exam'           // 考研·考公（topic: 'exam' | 'EXAM'）
  | 'marriage'       // 结婚·婚姻（topic: 'marriage' | 'MARRIAGE'）
  | 'jobChange'      // 工作·跳槽（topic: 'job' | 'JOB' | 'jobchange' | 'JOBCHANGE'）
  | 'inlaw'          // 婆媳·关系（topic: 'family' | 'FAMILY' | 'inlaw' | 'INLAW'）
  | 'invest'         // 投资·理财（topic: 'invest' | 'INVEST' | 'wealth' | 'WEALTH'）
  // 命盘卡片（6 个）
  | 'card-minggeSummary'    // 命格總評（sectionKey: 'minggeSummary'）
  | 'card-yongshenPattern'  // 用神格局（sectionKey: 'yongshenPattern'）
  | 'card-guancaiPattern'    // 官財格局（sectionKey: 'guancaiPattern'）
  | 'card-energyFlow'       // 能量流通（sectionKey: 'energyFlow'）
  | 'card-palaceSixKin'     // 宫位六亲（sectionKey: 'palaceSixKin'）
  | 'card-luckRhythm'        // 行运节奏（sectionKey: 'luckRhythm'）
  | 'card-shensha'           // 神煞解读（shenShaCode 存在时）
  | 'card-overview'          // 其他总览卡片（sectionKey 存在但不在上述列表时的兜底场景）
  // 通用模式
  | 'chat-general';          // 普通聊天（无 topic & 无 sectionKey）
 ...
 */
export const FOLLOWUP_SCENE_HINT: Record<FollowupScene, string> = {
  // 场景卡片
 ...
 */
export function resolveFollowupScene(params: {
  topic?: string;
  sectionKey?: string;
  shenShaCode?: string;
}): FollowupScene {
