 ...
 * @param userId 用戶 ID
 */
export async function requestDeletion(userId: string): Promise<DeletionRequestResponse> {
  const pool = getPool();
  
  // 1. 查詢當前用戶狀態
  const [rows]: any = await pool.execute(
    `SELECT user_id, status, delete_scheduled_at FROM users WHERE user_id = ?`,
    [userId]
  );
  
  if (rows.length === 0) {
    throw ApiError.notFound('用戶不存在', 'USER_NOT_FOUND');
  }
  ...
  if (user.status === 'DELETED') {
    throw ApiError.badRequest('帳號已刪除', 'ACCOUNT_ALREADY_DELETED');
  }
  ...
  if (result.affectedRows === 0) {
    // 並發情況：狀態已被其他請求改變
    throw ApiError.conflict('狀態已改變，請重試', 'STATE_CHANGED');
  }
  
  // 4. 查詢更新後的計劃刪除時間
  const [updatedRows]: any = await pool.execute(
    `SELECT delete_scheduled_at FROM users WHERE user_id = ?`,
    [userId]
  );
 ...
 * @param userId 用戶 ID
 */
export async function getDeletionStatus(userId: string): Promise<DeletionStatusResponse> {
  const pool = getPool();
  
  const [rows]: any = await pool.execute(
    `SELECT status, delete_scheduled_at FROM users WHERE user_id = ?`,
    [userId]
  );
 ...
 * @param userId 用戶 ID
 */
export async function cancelDeletion(userId: string): Promise<DeletionCancelResponse> {
  const pool = getPool();
  ...
  if (result.affectedRows === 0) {
    ...
    if (user.status === 'ACTIVE') {
      // 用戶已經是 ACTIVE，不需要撤銷
      throw ApiError.conflict('帳號狀態正常，無需撤銷', 'CANNOT_CANCEL_DELETION_INVALID_STATE');
    }
    
    if (user.status === 'DELETED') {
      throw ApiError.conflict('帳號已刪除，無法撤銷', 'CANNOT_CANCEL_DELETION_INVALID_STATE');
    }
    
    if (user.status === 'PENDING_DELETE') {
      // 狀態是 PENDING_DELETE 但更新失敗，說明已過期
      throw ApiError.conflict('已超過可撤銷期限', 'CANNOT_CANCEL_DELETION_EXPIRED');
    }
    
    // 其他情況
    throw ApiError.conflict('無法撤銷註銷', 'CANNOT_CANCEL_DELETION_INVALID_STATE');
  }
