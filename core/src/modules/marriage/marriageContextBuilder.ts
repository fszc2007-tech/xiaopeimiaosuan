...
import { calculateMarriagePattern } from './marriagePatternCalculator';

/**
 * 检查用户问题中是否提到伴侣或婚姻状态
 */
function detectMarriageMode(question: string): MarriageChatContext['mode'] {
  const keywords = {
  ...
  for (const [mode, keys] of Object.entries(keywords)) {
    if (keys.some(key => question.includes(key))) {
      return mode as MarriageChatContext['mode'];
    }
  }
 ...
 * 提取关系状态提示
 */
function extractRelationStatusHint(mode: MarriageChatContext['mode'], question: string): string {
  const modeMap: Record<MarriageChatContext['mode'], string> = {
    single: '未婚单身',
 ...
 * 提取基础信息（复用恋爱桃花的逻辑）
 */
function extractBasicInfo(chartResult: any): MarriageChatContext['basic'] {
 ...
 * 构建配偶宫信息（复用恋爱桃花的逻辑）
 */
async function buildSpousePalace(chartResult: any): Promise<MarriageChatContext['spouse']['spousePalace']> {
 ...
 * 构建配偶特征提示（基于配偶星和配偶宫）
 */
function buildSpouseProfileHint(
...
async function buildFortuneContext(
  ...
  spouseBranch: string,
  now: Date
): Promise<MarriageChatContext['fortune']> {
  const analysis = chartResult.analysis || {};
  ...
  const marriageWindows: MarriageChatContext['fortune']['marriageWindows'] = [];
  const filteredYears = annualBrief
    .filter((y: any) => y.year >= currentYear && y.year <= currentYear + 5)
  ...
  for (const item of filteredYears) {
    ...
    // 判断婚姻友好度
    let favourLevel: 'golden' | 'good' | 'normal' | 'hard' = 'normal';
    let type = '';
    let reason = '';
 ...
 * 提取辅助分析信息（复用恋爱桃花的逻辑）
 */
function extractExtraInfo(chartResult: any): MarriageChatContext['extra'] {
  const analysis = chartResult.analysis || {};
 ...
 */
export async function buildMarriageChatContext(params: {
  chartResult: any;
  ...
  // 5. 计算婚姻模式（核心逻辑）
  const marriagePattern = calculateMarriagePattern({
    W,
    spouseStarStatus,
    spousePalace,
