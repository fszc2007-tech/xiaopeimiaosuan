 ...
 * 检查神煞
 */
function checkShenSha(pillars: any, shenshaName: string): boolean {
  ...
  for (const pillarName of allPillars) {
    ...
    if (shenshaList.some((s: string) => s.includes(shenshaName))) {
      return true;
    }
  }
  return false;
 ...
 * 归一化日主强弱分数（统一为 0-1）
 */
function normalizeDayMasterScore(dayMasterStrength: any): number {
 ...
 * 计算早婚/晚婚/不婚倾向
 */
export function calculateMarriageTendency(params: {
  spouseStarStatus: any;      // 配偶星状态
  spousePalace: any;          // 配偶宫关系
  W: any;                     // 十神权重
  pillars: any;               // 四柱信息
  dayMasterStrength: any;     // 日主强弱
}): string {
  ...
  // 早婚倾向加分
  if (hasHongLuan || hasTianXi) score += 2;
  if (spouseStarStrong && !spouseStarStatus.conflict) score += 2;
  if (!spousePalaceChong) score += 1;
  if (biJieWeight < 0.2) score += 1;
  
  // 晚婚倾向加分
  if (hasGuChen || hasGuaSu) score -= 2;
  if (spouseStarWeak || spouseStarStatus.conflict) score -= 2;
  if (spousePalaceChong) score -= 1;
  if (biJieWeight > 0.4) score -= 1;
  if (shiShangWeight > 0.4) score -= 1;
  if (dayMasterWeak) score -= 1;
 ...
 * 计算婚姻稳定度
 */
export function calculateStabilityLevel(params: {
  spouseStarStatus: any;      // 配偶星状态
  spousePalace: any;          // 配偶宫关系
  breaking: any;              // 破格因素
  purityLevel: string;        // 清浊度
}): '偏稳' | '有波动' | '波动较大' {
  ...
  // 配偶星混杂扣分
  if (spouseStarStatus.mixed) score -= 2;
  ...
  // 配偶宫被冲扣分
  if ((spousePalace.relations?.chong || []).length > 0) score -= 2;
  
  // 配偶宫被刑害扣分
  if ((spousePalace.relations?.xing || []).length > 0) score -= 1;
  if ((spousePalace.relations?.hai || []).length > 0) score -= 1;
  
  // 破格因素扣分
  const factors = breaking?.factors || [];
  if (factors.some((f: any) => f.type?.includes('比劫夺财'))) score -= 2;
  if (factors.some((f: any) => f.type?.includes('食伤制官'))) score -= 2;
  if (factors.some((f: any) => f.type?.includes('官杀混杂'))) score -= 1;
  ...
  // 配偶宫有合加分（稳定）
  if ((spousePalace.relations?.he || []).length > 0) score += 1;
  if ((spousePalace.relations?.sanhe || []).length > 0) score += 1;
 ...
 * 计算矛盾处理风格
 */
export function calculateConflictStyle(params: {
  W: any;                     // 十神权重
  dayMasterStrength: any;     // 日主强弱
  shiShangStatus: any;        // 食伤状态
}): string {
  ...
  const styles: Array<{ type: string; weight: number }> = [];
 ...
 * 计算现实压力落点
 */
export function calculateRealityPressureFocus(params: {
  W: any;                     // 十神权重
  structure: any;             // 格局结构
}): string {
  ...
  const pressures: Array<{ type: string; weight: number }> = [];
 ...
 * 计算风险提示
 */
export function calculateMarriageRiskHints(params: {
  breaking: any;              // 破格因素
  spouseStarStatus: any;      // 配偶星状态
  spousePalace: any;          // 配偶宫关系
  conflictStyle: string;      // 矛盾处理风格
}): string[] {
  ...
  // 从破格因素提取（复用恋爱桃花的逻辑）
  if (factors.some((f: any) => f.type?.includes('比劫夺财'))) {
    hints.push('容易遇到感情竞争，需要主动争取和维护关系');
  }
  if (factors.some((f: any) => f.type?.includes('食伤制官'))) {
    hints.push('容易因为表达方式不当而产生矛盾，需要注意沟通方式');
  }
  if (factors.some((f: any) => f.type?.includes('官杀混杂'))) {
 ...
 * 计算改善方向
 */
export function calculateHealingHints(params: {
  riskHints: string[];        // 风险提示
  conflictStyle: string;       // 矛盾处理风格
  yongshen: any;              // 喜用神
  realityPressureFocus: string; // 现实压力落点
}): string[] {
 ...
 * 统一计算婚姻模式
 */
export function calculateMarriagePattern(params: {
  W: any;                     // 十神权重
  spouseStarStatus: any;      // 配偶星状态
  spousePalace: any;          // 配偶宫关系
  breaking: any;              // 破格因素
