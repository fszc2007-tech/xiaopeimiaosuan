 ...
 * 提取基础信息
 */
function extractBasicInfo(chartResult: any): JobChangeChatContext['basic'] {
 ...
 * 推导适合体制/大机构程度
 */
function deriveFitSystem(chartResult: any): '低' | '中' | '高' {
  const analysis = chartResult.analysis || {};
 ...
 * 推导适合企业程度
 */
function deriveFitEnterprise(chartResult: any): '低' | '中' | '高' {
  const analysis = chartResult.analysis || {};
  const structure = analysis.structure || {};
  const W = structure.W || structure.tenGodWeights || {};
  const structureName = structure.name || '';
 ...
 * 推导适合小公司/创业/不确定性高程度
 */
function deriveFitStartup(chartResult: any): '低' | '中' | '高' {
  const analysis = chartResult.analysis || {};
  const structure = analysis.structure || {};
  const W = structure.W || structure.tenGodWeights || {};
  const structureName = structure.name || '';
 ...
 * 推导适合自由职业/创业程度
 */
function deriveFitFreelanceOrBiz(chartResult: any): '低' | '中' | '高' {
  const analysis = chartResult.analysis || {};
  const structure = analysis.structure || {};
  const W = structure.W || structure.tenGodWeights || {};
  const dogongResult = analysis.dogongResult || analysis.dogong || {};
 ...
 * 推导角色偏好
 */
function deriveRolePreference(chartResult: any): '技术专家型' | '管理型' | '综合协调型' | '创意表达型' {
  const analysis = chartResult.analysis || {};
  const structure = analysis.structure || {};
  const W = structure.W || structure.tenGodWeights || {};
  ...
  return weights[0].type as any;
}

/**
 * 推导工作风格标签
 */
function deriveWorkStyleTags(chartResult: any, careerFit: JobChangeChatContext['careerFit']): string[] {
  const analysis = chartResult.analysis || {};
  const structure = analysis.structure || {};
  const W = structure.W || structure.tenGodWeights || {};
 ...
 * 推导职业匹配倾向
 */
function deriveCareerFit(chartResult: any): JobChangeChatContext['careerFit'] {
 ...
 * 推导稳定偏好
 */
function deriveStabilityPreference(chartResult: any): '偏稳' | '平衡' | '偏变动' {
  const analysis = chartResult.analysis || {};
  const guancaiPattern = analysis.guancaiPattern || {};
  const stability = guancaiPattern?.stability?.career;
  const structure = analysis.structure || {};
 ...
 * 推导矛盾处理风格
 */
function deriveConflictStyle(chartResult: any): string {
  const analysis = chartResult.analysis || {};
  const structure = analysis.structure || {};
  const W = structure.W || structure.tenGodWeights || {};
 ...
 * 推导与上级/规则的关系
 */
function deriveAuthorityRelationHint(chartResult: any): string {
  const analysis = chartResult.analysis || {};
  const structure = analysis.structure || {};
  const W = structure.W || structure.tenGodWeights || {};
 ...
 * 推导钱/成长/生活平衡
 */
function deriveMoneyVsGrowthPreference(chartResult: any): string {
  const analysis = chartResult.analysis || {};
  const structure = analysis.structure || {};
  const W = structure.W || structure.tenGodWeights || {};
  const guancaiPattern = analysis.guancaiPattern || {};
 ...
 * 推导风险提示
 */
function deriveRiskHints(chartResult: any, workPattern: JobChangeChatContext['workPattern']): string[] {
  const analysis = chartResult.analysis || {};
  const guancaiPattern = analysis.guancaiPattern || {};
  ...
  // 从官财格局的风险因素中提取
  riskFactors.forEach((tag: string) => {
 ...
 * 推导职场模式
 */
function deriveWorkPattern(chartResult: any): JobChangeChatContext['workPattern'] {
 ...
 * 构建行运信息
 */
function buildFortuneInfo(chartResult: any, now: Date): JobChangeChatContext['fortune'] {
  const analysis = chartResult.analysis || {};
  ...
  function mapFavourLevelToJob(favLevel: string, shishen?: string): 'golden' | 'good' | 'normal' | 'hard' {
  ...
  // 生成跳槽窗口类型
  function generateJobWindowType(item: any): string {
  ...
  // 生成跳槽窗口原因
  function generateJobWindowReason(item: any): string {
    const shishen = item.shishen || '';
  ...
  const jobWindows = annualBrief
    .filter((item: any) => item.year >= currentYear && item.year <= currentYear + 5)
    .slice(0, 5)
    .map((item: any) => ({
      year: item.year,
 ...
 * 正则兜底提取用户计划
 */
function extractPlanWithRegex(question: string): JobChangeChatContext['plan'] {
  // 提取当前状态
  ...
  if (/互联网|IT|软件/.test(question)) currentIndustry = '互联网';
  if (/制造|工厂/.test(question)) currentIndustry = '制造';
  if (/教育|教师/.test(question)) currentIndustry = '教育';
  if (/金融|银行/.test(question)) currentIndustry = '金融';
  if (/公务员|体制/.test(question)) currentIndustry = '公务员';
  
  // 提取岗位
  let currentRole: string | undefined;
  if (/技术|开发|程序员/.test(question)) currentRole = '技术';
  if (/产品/.test(question)) currentRole = '产品';
  if (/运营/.test(question)) currentRole = '运营';
  if (/销售/.test(question)) currentRole = '销售';
  if (/行政/.test(question)) currentRole = '行政';
  if (/财务|会计/.test(question)) currentRole = '财务';
  if (/教师/.test(question)) currentRole = '教师';
  
  // 提取痛点（简单提取）
  let currentPainPoints: string | undefined;
  ...
  // 提取目标变动类型
  let targetChangeType: string | undefined;
  if (/加薪|涨薪/.test(question)) targetChangeType = '加薪跳槽';
  if (/换行业|转行/.test(question)) targetChangeType = '换行业';
  if (/换城市|搬家/.test(question)) targetChangeType = '换城市';
  if (/转岗位|转岗/.test(question)) targetChangeType = '转岗位';
  if (/回老家|回家/.test(question)) targetChangeType = '回老家';
  if (/创业|自由职业|freelance/.test(question)) targetChangeType = '创业 / 自由职业';
  
  // 提取风险承受度
  let riskTolerance: '偏保守' | '平衡' | '偏激进' = '平衡';
  if (/保守|稳妥|稳定/.test(question)) riskTolerance = '偏保守';
  if (/激进|冒险|大胆/.test(question)) riskTolerance = '偏激进';
...
function validatePlanData(data: any): boolean {
  ...
  return (
    validStatus.includes(data.currentStatus) &&
...
async function extractPlanFromQuestion(
  question: string
): Promise<JobChangeChatContext['plan']> {
  // 1. 构建 Few-Shot 示例
 ...
 * 获取默认计划
 */
function getDefaultPlan(): JobChangeChatContext['plan'] {
  return {
    currentStatus: '在职想观望',
    hasOffer: false,
    riskTolerance: '平衡',
 ...
 * 提取辅助分析信息
 */
function extractExtraInfo(chartResult: any): JobChangeChatContext['extra'] {
  const analysis = chartResult.analysis || {};
  const guancaiPattern = analysis.guancaiPattern || {};
 ...
 * 判断模式
 */
function detectMode(plan: JobChangeChatContext['plan']): 'working' | 'betweenJobs' | 'student' | 'freelance' | 'business' {
 ...
 * 判断职业阶段提示
 */
function deriveCareerStageHint(chartResult: any, now: Date): string {
  const analysis = chartResult.analysis || {};
  const luckRhythm = analysis.luckRhythm || {};
 ...
 */
export async function buildJobChangeChatContext(params: {
  chartResult: any;
