 ...
 * 提取基础信息
 */
function extractBasicInfo(chartResult: any): InLawChatContext['basic'] {
...
async function buildSpouseAndExpression(
...
): Promise<InLawChatContext['spouseAndExpression']> {
  const analysis = chartResult.analysis || {};
  ...
  const spouseStarType: '财星' | '官杀' = isMale ? '财星' : '官杀';
  
  // 构建配偶星分布
  const utilsModule = await import('../../../engine/analysis/utils.js');
  const { getShishenPositions } = utilsModule;
  
  const spouseStarPositions = getShishenPositions(
    pillars,
    dayMasterGan,
    spouseStarType === '财星' ? ['cai', 'pCai'] : ['guan', 'sha']
  );
 ...
 * 复用恋爱专线的逻辑
 */
async function buildPalaceContext(chartResult: any): Promise<InLawChatContext['palace']> {
  const pillars = chartResult.pillars || {};
 ...
 * 提取清浊调候信息
 */
function extractPatternInfo(chartResult: any): InLawChatContext['patternAndBearing'] {
  const analysis = chartResult.analysis || {};
...
async function buildFortuneContext(
  ...
  spouseBranch: string,
  now: Date
): Promise<InLawChatContext['fortune']> {
  const analysis = chartResult.analysis || {};
  ...
  // 提取当前大运信息
  const currentLuck: InLawChatContext['fortune']['currentLuck'] = {
  ...
  const years: InLawChatContext['fortune']['years'] = [];
 ...
 * 提取辅助分析信息
 */
function extractExtraInfo(chartResult: any): InLawChatContext['extra'] {
  const analysis = chartResult.analysis || {};
 ...
 * 构建婆媳关系聊天上下文
 */
export async function buildInLawChatContext(params: {
  chartResult: any;
  gender: 'male' | 'female' | 'unknown';
  userQuestion?: string;
  now?: Date;
