...
export async function getConversations(params: {
  userId: string;
  masterIds?: string[]; // 筛选：按命主
  dateFilter?: 'today' | 'week' | 'month' | 'all'; // 筛选：按日期
  page?: number;
  pageSize?: number;
}): Promise<{
  ...
  const [countRows]: any = await pool.execute(countSql, countParams);
  ...
  const [rows]: any = await pool.execute(listSql, queryParams);
 ...
 * 获取命主列表（用于筛选）
 */
export async function getMastersForFilter(params: {
  userId: string;
}): Promise<Array<{
  masterId: string;
  masterName: string;
