...
export async function getBillingSummary(params: {
  ...
  const queryParams: any[] = [queryStart, queryEnd];
...
export async function getBillingTrend(params: {
  ...
  if (daysDiff <= 3) {
    ...
    if (provider) dailyParams.push(provider);
    if (model) dailyParams.push(model);
    queryParams.splice(0, queryParams.length, ...dailyParams);
  }
  
  const [rows]: any = await pool.execute(query, queryParams);
  
  const items: BillingTrendItemDto[] = rows.map((row: any) => ({
 ...
 * 按模型聚合统计
 */
export async function getBillingByModel(params: {
  startDate?: string;
  endDate?: string;
  provider?: string;
  ...
  // 检查 daily 表是否有数据
  const [dailyCheck]: any = await pool.execute(
    `SELECT COUNT(*) AS count FROM llm_usage_daily 
     WHERE date >= ? AND date <= ?`,
  ...
  const items: BillingByModelItemDto[] = rows.map((row: any) => ({
