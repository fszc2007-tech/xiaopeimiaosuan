 ...
 * 十神对应的五行映射
 */
const SHISHEN_TO_ELEMENT: Record<string, string> = {
  '比肩': 'same',      // 同我（日主五行）
  '劫财': 'same',      // 同我（日主五行）
  '食神': 'generates', // 我生
  '伤官': 'generates', // 我生
...
function determineFavorStatus(
  ...
  try {
    ...
    const MOTHER_OF: Record<string, string> = {
      '木': '水', '火': '木', '土': '火', '金': '土', '水': '金'
    };
    const CONTROLLER_OF: Record<string, string> = {
      '木': '金', '火': '水', '土': '木', '金': '火', '水': '土'
    };
    ...
    // 获取十神对应的五行
    let shishenElement: string | null = null;
 ...
 */
export async function getShishenReading(
  shishenCode: string,
  pillarType: PillarType | undefined,
  gender: GenderType,
  baziData?: any
): Promise<ShishenReadingDto | null> {
  const pool = getPool();
  ...
  if (pillarType) {
    ...
    const [genderRows]: any = await pool.execute(
      `SELECT * FROM shishen_readings 
       ...
       LIMIT 1`,
      [shishenCode, pillarType, gender]
    );
    
    if (genderRows.length > 0) {
      const row: ShishenReadingRow = genderRows[0];
    ...
    const row: ShishenReadingRow = allRows[0];
    const favorStatus = baziData 
      ? determineFavorStatus(row.name, baziData)
      : undefined;
    ...
    if (genderRows.length > 0) {
      const firstRow: ShishenReadingRow = genderRows[0];
      const favorStatus = baziData 
      ...
      return {
        ...
        pillar_explanation: genderRows.map((row: ShishenReadingRow) => ({
          pillar: row.pillar_type,
          text: row.for_this_position,
        })),
    ...
    const firstRow: ShishenReadingRow = allRows[0];
    const favorStatus = baziData 
      ? determineFavorStatus(firstRow.name, baziData)
      : undefined;
    ...
    return {
      ...
      type: firstRow.type,
      short_title: firstRow.short_title || '',
      pillar_explanation: allRows.map((row: ShishenReadingRow) => ({
        pillar: row.pillar_type,
        text: row.for_this_position,
      })),
      recommended_questions: firstRow.recommended_questions || [],
