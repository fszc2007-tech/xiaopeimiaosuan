...
import { checkProStatus, clearProStatusCache } from './requirePro';
import { isProFeatureGated } from '../services/systemConfigService';
 ...
 * 创建 Pro 功能检查中间件
 */
export function requireProFeature(featureKey: FeatureKey) {
  return async (req: Request, res: Response, next: NextFunction): Promise<void> => {
    try {
      ...
      const requiresPro = await isProFeatureGated(featureKey);
      if (!requiresPro) {
        // 该功能无需 Pro，直接通过
        return next();
      }
 ...
 * 获取用户可用功能列表（工具函数）
 */
export async function getUserAvailableFeatures(userId: string): Promise<{
  isPro: boolean;
  ...
  const allFeatures: FeatureKey[] = ['shensha', 'overview', 'advanced_chat'];
  const lockedFeatures: string[] = [];
  const availableFeatures: string[] = [];

  for (const feature of allFeatures) {
    const isLocked = await isProFeatureGated(feature);
    if (isLocked) {
