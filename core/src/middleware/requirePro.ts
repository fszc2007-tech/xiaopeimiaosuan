...
const proStatusCache = new Map<string, ProStatusCache>();
...
export function checkProStatus(
  ...
  proPlan?: 'yearly' | 'monthly' | 'quarterly' | 'lifetime' | null
): boolean {
  // 必须 is_pro = TRUE
  if (!isPro) {
    return false;
 ...
 * @param userId 用户 ID
 */
export function clearProStatusCache(userId: string): void {
