...
import {
  ...
  setDefaultLLM,
  testLLMConnection,
} from '../../modules/admin/llmConfigService';
...
router.get('/:provider', async (req: Request, res: Response) => {
  try {
    const { provider } = req.params as { provider: 'deepseek' | 'chatgpt' | 'qwen' };
...
router.put('/:provider', async (req: Request, res: Response) => {
  try {
    ...
    const config = await updateLLMConfig(provider, req.body);
 ...
 * 设置默认 LLM
 */
router.post('/:provider/set-default', async (req: Request, res: Response) => {
  try {
    const { provider } = req.params as { provider: 'deepseek' | 'chatgpt' | 'qwen' };
