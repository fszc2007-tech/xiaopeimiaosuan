...
export const regionConfig = {
  ...
  CN: {
    ...
    loginMethods: ['phone', 'wechat'] as const,
    requirePhone: true,
    requireEmail: false,
  },
...
export function validateAuthConfig(): void {
  ...
  if (otpConfig.ttlMinutes < 1 || otpConfig.ttlMinutes > 60) {
    throw new Error('[Auth Config] OTP TTL must be between 1 and 60 minutes');
  }
  
  if (otpConfig.sendIntervalSeconds < 30 || otpConfig.sendIntervalSeconds > 300) {
    throw new Error('[Auth Config] Send interval must be between 30 and 300 seconds');
  }
  
  if (otpConfig.dailyLimit < 1 || otpConfig.dailyLimit > 100) {
