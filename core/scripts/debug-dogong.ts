...
import { analyzeDogong } from '../engine/analysis/dogong.js';
...
const testAnalysis = {
  dayMaster: { gan: '庚' },
  strength: computeDayMasterStrength(testPillars, { school: 'ziping' })
};

async function diagnoseDogong() {
  console.log('🔍 开始诊断做功分析...\n');
  
  const result = analyzeDogong(testPillars, testAnalysis, { maxPaths: 10 });
  ...
  keyRelations.forEach(rel => {
    const found = result.graph.edges.some(e => 
      e.from === rel.from && e.to === rel.to && e.relation === rel.type
    );
    if (found) {
  ...
  if (dayMasterEdges.length === 0) {
    ...
    console.log('从"日主"的边:', dayMasterEdges.map(e => 
