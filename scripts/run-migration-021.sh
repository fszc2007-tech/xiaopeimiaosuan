...
# 执行数据库迁移 021: 插入金神的解读内容
# 
# 使用方法：
# chmod +x scripts/run-migration-021.sh
# ./scripts/run-migration-021.sh

set -e

echo "=========================================="
echo "执行数据库迁移: 021_insert_jin_shen_readings.sql"
echo "=========================================="
echo ""

# 检查迁移文件是否存在
MIGRATION_FILE="core/src/database/migrations/021_insert_jin_shen_readings.sql"
...
if [ -n "$DB_PASSWORD" ]; then
  ...
  mysql -h "$DB_HOST" --default-character-set=utf8mb4 --default-character-set=utf8mb4 -P "$DB_PORT" -u "$DB_USER" -p "$DB_NAME" < "$MIGRATION_FILE"
fi

echo ""
echo "✅ 迁移完成！"
echo ""
echo "验证: 检查 shensha_readings 表中是否有金神的解读记录"
echo ""
